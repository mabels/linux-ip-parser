...
impl Interface {
    ...
    fn as_commands(&self, direction: &str) -> Vec<String> {
        ...
        let ret = self.ips
            .iter()
            .map(|ip| format!("ip addr {} {} dev {}", direction, ip, self.name))
            .collect();
        //   println!("0002");
        //   for ip in &ret {
...
pub fn parse_from_string(lines: &String) -> IpAddr {
    let mut ip_addr = IpAddr { interfaces: Vec::new() };
            ...
            Some(line) => {
                ...
                if re_line.is_match(line) {
                    ...
                    for cap in re_mac_address.captures_iter(line) {
                        ip_addr.interfaces[op_iface]
                            .set_mac_address(&cap.at(1).unwrap().to_string());
                    }
